...
export function jobBelongsToMainConversation(job) {
  if (!job) return isMainAgentId(undefined);
  if (job.loopId || job.source === "loop") return false;
...
/**
 * Drop user + assistant turns for a job that never ran (queued delete).
 * @param {ConversationState} state
 * @param {string} jobId
 */
export function removeJobFromConversation(state, jobId) {
  if (!state || !jobId) return state;
  const id = String(jobId);
  state.turns = (state.turns || []).filter((t) => String(t?.jobId || "") !== id);
  state.updatedAt = new Date().toISOString();
  return state;
}
